...
function Supports(props) {
  const { game, constrain, linkConstrain } = props;
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force("charge", d3.forceManyBody().strength(constrain ? -200 : -300))
      .force("center", d3.forceCenter(width / 2, height / 2))
      .force("x", constrain ? d3.forceX(width / 2).strength(0.05) : null)
      .force("y", constrain ? d3.forceY(height / 2).strength(0.05) : null)
      .on("tick", ticked);
  ...
  }, [data, windowWidth, constrain, linkConstrain]);

  useEffect(() => {
    fetch(`/api/relationships/supports?game=${game}`, {
      method: "GET",
      mode: "cors",
      headers: {
